...
def payload_data():
    ...
    return {
        ...
        "propellant_mass": np.linspace(10, 0, 100),  # Simulated propellant mass data
        "burn_time": 5,
        "chamber_length": 0.5,
        "outer_diameter": 0.1,
        "motor_mass": 5.0,
...
def test_eng_header_format(payload_data):
    ...
    total_mass = (
        f"{payload_data['propellant_mass'][0] + payload_data['motor_mass']:.4f}"
    )
    assert header.startswith(expected_start), "Header start format is incorrect."
    assert initial_mass in header, "Initial propellant mass is missing or incorrect."
    assert total_mass in header, "Total mass is missing or incorrect."
    assert payload_data["manufacturer"] in header, "Manufacturer name is missing."
...
def test_eng_data_format(payload_data):
    ...
    lines = content.strip().split("\n")
    data_lines = [line for line in lines if not line.startswith(";")][1:]  # Skip header
...
def test_eng_file_ends_with_semicolon(payload_data):
    ...
    assert content.strip().endswith(";"), "The .eng file must end with a semicolon."
