...
Provides fixtures that parametrize tests across all FMM2D and FMM3D geometries.
"""
...
def create_conical_segment_small_core(
    length=1.0,
    outer_diameter=0.1,
    density_ratio=1.0,
):
    """Factory for creating ConicalGrainSegment with small core (more propellant)."""
    return ConicalGrainSegment(
        length=length,
        outer_diameter=outer_diameter,
        upper_core_diameter=0.02,
        lower_core_diameter=0.02,
        density_ratio=density_ratio,
    )


def create_conical_segment_large_core(
    length=1.0,
    outer_diameter=0.1,
    density_ratio=1.0,
):
    """Factory for creating ConicalGrainSegment with large core (less propellant)."""
    return ConicalGrainSegment(
        length=length,
        outer_diameter=outer_diameter,
        upper_core_diameter=0.06,
        lower_core_diameter=0.06,
        density_ratio=density_ratio,
    )
