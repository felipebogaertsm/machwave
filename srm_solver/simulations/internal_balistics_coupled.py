...
def run_ballistics(
    ...
    P_0, P_0_psi, P_exit = (
        np.array([igniter_pressure]),
        np.array([igniter_pressure]) * 1.45e-4,
        np.array([])
    )
    ...
    optimal_expansion_ratio = np.array([])
    ...
    C_f, C_f_ideal, T = np.array([]), np.array([]), np.array([])
    ...
    while y[i] >= 0 or m_prop[i - 1] > 0:
        ...
        if end_thrust is False:
            ...
            assert a > 0, (
                '\nCHAMBER PRESSURE OUT OF BOUNDS, change Propellant or '
            ...
            k2 = solve_cp_seidel(P_0[i] + 0.5 * k1 * d_t,
                P_ext[i],
                A_burn[i],
                V_0[i],
                structure.get_throat_area(),
            ...
            k3 = solve_cp_seidel(P_0[i] + 0.5 * k2 * d_t,
                P_ext[i],
                A_burn[i],
                V_0[i],
                structure.get_throat_area(),
            ...
            k4 = solve_cp_seidel(P_0[i] + 0.5 * k3 * d_t,
                P_ext[i],
                A_burn[i],
                V_0[i],
                structure.get_throat_area(),
            ...
            P_0_psi = np.append(P_0_psi, P_0[i] * 1.45e-4)
            ...
            n_kin_atual, n_tp_atual, n_bl_atual = (
