...
class Tank:
    ...
    def __init__(
        ...
        # Vapor viscosity, memoized per fluid mass and temperature.
        self._viscosity_by_mass: dict[tuple[float, float], float] = {}
        # Saturated liquid viscosity, memoized per temperature.
        self._saturated_liquid_viscosity_by_temperature: dict[float, float] = {}
    ...
    def get_saturated_liquid_viscosity(self, temperature: float) -> float:
        """
        Return the saturated liquid dynamic viscosity at a temperature [Pa-s].

        Resolved on demand rather than at construction, so a fluid CoolProp
        ...
        """
        if temperature not in self._saturated_liquid_viscosity_by_temperature:
            self._saturated_liquid_viscosity_by_temperature[temperature] = (
                self._get_coolprop_viscosity(
                    lambda: self._coolprop.get_saturated_liquid_viscosity(temperature)
                )
            )
        return self._saturated_liquid_viscosity_by_temperature[temperature]
    ...
    def get_dynamic_viscosity(
        ...
        A viscosity given to the tank stands for every state it reaches.
        Otherwise this follows the same fill states as `get_density`: a
        partially liquid tank delivers saturated liquid, and an all-vapor tank
        delivers vapor at the bulk density.
        ...
        temperature = self.get_temperature(fluid_mass, internal_energy)
        if self.is_delivering_liquid(fluid_mass, internal_energy):
            return self.get_saturated_liquid_viscosity(temperature)

        memo_key = (fluid_mass, temperature)
        if memo_key not in self._viscosity_by_mass:
            self._viscosity_by_mass[memo_key] = self._get_coolprop_viscosity(
                lambda: self._coolprop.get_viscosity_at_temperature_density(
                    temperature, fluid_mass / self.volume
                )
            )
        return self._viscosity_by_mass[memo_key]
