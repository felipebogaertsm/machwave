...
class BATES:
    ...
    def getOptimalSegmentLength(self):
    ...
    def getMassFluxPerSegment(self, r: float, pp, x):
...
def getExitPressure(k_2ph_ex, E, P0):
...
def getCircleArea(diameter: float):
    Area = np.pi * 0.25 * diameter ** 2
    return Area


def getChamberVolume(L_cc: float, D_in: float, V_prop: np.array):
...
def getThrustCoefficient(P0, P_external, P_exit, E, k_2ph_ex, n_cf):
...
def getImpulses(F_avg, t, m_prop):
    I_total = F_avg * t[-1]
...
def getOperationalCorrectionFactors(P0: list, Pe: float, P0psi: list, Isp_frozen: float, Isp_shifting: float,
