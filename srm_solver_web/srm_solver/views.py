...
def loginuser(request):
    ...
    if request.method == 'GET':
        # In case the user has just entered the page:
        return render(request, 'srm_solver/loginuser.html', {'log_form': AuthenticationForm()})
    else:  # in case the user has already posted the request
        # Checking if the user credentials are correct:
        user = authenticate(request, username=request.POST['username'], password=request.POST['password'])
        if user is None:  # in case the user does not exist, seng him back to log in page with an error
            return render(request, 'srm_solver/loginuser.html', {'form': AuthenticationForm(),
