...
Tanks are identified by CoolProp fluid name (e.g., `"N2O"`, `"Oxygen"`, `"Hydrogen"`). CoolProp has no viscosity model for some of them, nitrous oxide among them; those need `dynamic_viscosity` supplied to model anything that depends on it, such as a feed line pressure drop.
